...
type VariableOutputBoolNode struct {
	id       int
	nodeType string
	input    []OutputNodeHandle
}

func (d *VariableOutputBoolNode) GetId() int {
	return d.id
}

func (d *VariableOutputBoolNode) GetNodeType() string {
	return d.nodeType
}

func (d *VariableOutputBoolNode) InitNode(id_ int, nodeType_ string, output_ []OutputNodeHandle) {
	d.id = id_
...
func init() {
	...
	var digitalOutputDescription = nodeDescription{
		...
		Type_:         "variableOutputBool",
		Display:       "Output Bool ",
		Label:         "variable Output Bool",
		Stretchable:   false,
		Services:      services,
	...
	RegisterNodeCreator("variableOutputBool", func() (Node, error) {
		return &VariableOutputBoolNode{
			id:       -1,
			nodeType: "",
			input:    nil,
		}, nil
...
func (d *VariableOutputBoolNode) GetOutput(outName string) *OutputNodeHandle {
	for i, name := range d.input {
		if name.OutputHandle.Name == outName {
			if d.input[i].FriendlyName == "default" {
				...
				variable.AddOutput(d.input[i].Service, "0")
			}
			return &d.input[i]
		}
	}
...
func (d *VariableOutputBoolNode) GetOutputList() []OutputNodeHandle {
	return d.input
}
